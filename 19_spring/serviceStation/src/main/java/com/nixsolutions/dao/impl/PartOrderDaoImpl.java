...
public class PartOrderDaoImpl implements PartOrderDao {
	...
	public List<PartOrder> getPartsByOrderId(Integer order_id) {
		...
		try {
			part_orders = sessionFactory.getCurrentSession().createCriteria(PartOrder.class, "pOrder")
					.createAlias("pOrder.order", "order").add(Restrictions.eq("order.orderId", order_id)).list();
		} catch (Exception ex) {
			logger.error(ex);
		}
		return part_orders;
	...
	public PartOrder getPartByOrderId(Integer order_id, Integer part_id) {
		...
		try {
			part_order = (PartOrder) sessionFactory.getCurrentSession().createCriteria(PartOrder.class, "pOrder")
