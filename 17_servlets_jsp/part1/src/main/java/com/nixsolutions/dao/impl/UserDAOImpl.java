...
public class UserDAOImpl implements UserDAO<User> {
	...
	public boolean create(User t) {
		...
		try {
			ps = conn.prepareStatement("INSERT INTO user (username,password, role_id, customer_id) VALUES (?,?,?,?)");
	...
	public boolean update(User t) {
		...
		try {
			ps = conn.prepareStatement(
					"UPDATE user SET username=?, password=?, role_id=?, customer_id=? WHERE user_id=?");
			ps.setString(1, t.getUsername());
			ps.setString(2, t.getPassword());
			ps.setInt(3, t.getRole_id());
			ps.setInt(4, t.getCustomer_id());
