...
public class PartOrderDaoImpl implements PartOrderDao {
	...
	public PartOrder getPartByOrderId(Integer order_id, Integer part_id) {
		try (Connection connection = ConnectionManager.getConnection()){
			logger.debug("Create DB connector");
			String query = "SELECT * FROM SQLLAB.PART_ORDER WHERE order_id = ? AND part_id=?;";
			logger.trace("Send query \"" + query + "\"");
			...
			logger.trace("Generate list of the sqllab.part objects");
			while (set.next()) {
				return new PartOrder(set.getInt("order_id"), set.getInt("part_id"), set.getInt("amount"));
			}
			stmt.close();
		} catch (SQLException | ClassNotFoundException e) {
			logger.error(e.getMessage(), e);
	...
	public void updatePartOrder(Integer order_id, Integer part_id, Integer amount) {
		try (Connection connection = ConnectionManager.getConnection()){
			String query = "UPDATE sqllab.part_order SET amount = ? WHERE order_id =? AND part_id =?";
			logger.trace("Send query \"" + query + "\"");
			...
			if (set == 1)
				...
				logger.error("New part_id was not added to order");
			stmt.close();
		} catch (SQLException | ClassNotFoundException e) {
			logger.error(e.getMessage(), e);
		} 
