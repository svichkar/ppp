...
public class ClientDaoH2 implements ClientDao {
    ...
    public void create(Client client) {
        try (Connection conn
            ...
            PreparedStatement newClient = conn.prepareStatement(
                    "INSERT INTO client (first_name, last_name, web_user_id) VALUES (?,?,?)");
            newClient.setString(1, client.getFirstName());
            newClient.setString(2, client.getLastName());
            if (client.getWebUserId() != null) {
                newClient.setLong(3, client.getWebUserId());
            } else {
                newClient.setNull(3, Types.BIGINT);
            }
    ...
    public void update(Client client) {
        try (Connection conn
                = ConnectionManagerH2.getConnection()) {
            PreparedStatement newClient = conn.prepareStatement(
                    "UPDATE client SET first_name = ?, last_name = ?, web_user_id =? WHERE client_id = ?");
            newClient.setString(1, client.getFirstName());
            newClient.setString(2, client.getLastName());
            if (client.getWebUserId() != null) {
                newClient.setLong(3, client.getWebUserId());
            ...
            newClient.setLong(4, client.getClientId());
            newClient.executeUpdate();
            newClient.close();
        } catch (SQLException | RuntimeException ex) {
            LOGGER.error(ex);
    ...
    public Client findClientById(long clientId) {
        ...
        try (Connection conn
            ...
            if (searchResults.next()) {
                ...
                searchedClient.setWebUserId(searchResults.getLong("web_user_id"));
            } else {
