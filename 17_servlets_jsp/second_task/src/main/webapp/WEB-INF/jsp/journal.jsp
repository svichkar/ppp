	...
	<jsp:attribute name="content_area">
       <h2>Journal</h2>
        <table class="centered-table" border="1">
            ...
            <tbody>            
            <tr>
                    <td><c:out value="${student.student.firstName}" /></td>
                    <td><c:out value="${student.student.lastName}" /></td>
                    <td><c:out
							value="${student.student.admissionDate}" /></td>
		...
		<form action="journal" method="post">
         <input type="hidden" name="student_id"
				value="<c:out value="${student.student.studentId}"/>">
       ...
       <option value="<c:out value="${term.termId}"/>"><c:out
	    ...
	    <br /><input type="submit" name="viewJournal" value="View Journal">
       </form>
       <table class="centered-table" border="1">
            <thead>
                <tr>
            ...
            <tr>
            		<td><c:out value="${journal.term.termName}" /></td>
                    <td><c:out
								value="${journal.subject.subjectName}" /></td>
                    <td><c:out value="${journal.grade.gradeName}" /></td>
                    <td><c:out value="${journal.gpa.gradeName}" /></td>                  
                </tr>
            </c:forEach>
            </tbody>
        </table>             
