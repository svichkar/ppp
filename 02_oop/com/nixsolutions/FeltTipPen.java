 ...
 */
public class FeltTipPen extends WritingInstrument {
    ...
    /**
     * Calls the local method to ensure that the cap removed, so that the item
     * is able to write.
     */
    @Override
    ...
    protected void write(StringBuilder sb) {
        ...
        for (int i = 0; i < sb.length(); i++) {
            if (pigmentLeftPercent.compareTo(pigmentPercentPerChar) >= 0) {
                ...
                pigmentLeftPercent = pigmentLeftPercent
                        .subtract(pigmentPercentPerChar);
                charsWritten++;
            } else {
                System.out.print(
    ...
    /**
     * Sets the percentage of used pigment per character written, according to
     * the specified logic of the item.
     */
