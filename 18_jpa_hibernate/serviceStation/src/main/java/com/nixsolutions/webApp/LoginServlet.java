...
public class LoginServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		/*
		 * try { //DropDB.dropDB(); //CreateDB.createDB();
		 * //FillingDB.fillingDB(); } catch (ClassNotFoundException e) {
		 * logger.error(e); }
		 */
	}
	 ...
	 *//**
		 ...
		 *//*
			 ...
			 * IOException { // response.getWriter().append("Served at: //
			...
			if (request.getSession().getAttribute("login") != null) {
				if (!request.getSession().getAttribute("login").equals(login)) {
					...
					if (userDao.validate(login, password)) {
						...
						// userDao.getUser().getUser_role_id());
						 response.sendRedirect(selectNextPage(userDao.getUser().getUserRole().getUser_role_id()));
						/* request.getRequestDispatcher(selectNextPage(userDao.getUser().getUserRole().getUser_role_id()))
				...
				if (userDao.validate(login, password)) {
					...
					// response.sendRedirect(selectNextPage(userDao.getUser().getUser_role_id()));
					 request.getRequestDispatcher(selectNextPage(userDao.getUser().getUserRole().getUser_role_id())).forward(request,
					 response);
				} else {
					request.getSession().removeAttribute("login");
					request.getSession().removeAttribute("user_id");
