...
public abstract class AbstractDaoImpl {
    ...
    protected <T> Boolean insert(T obj, Map<String, String> colNamesMap, String tableName) {
	try (Connection conn = ConnectionManager.getConnection()) {
		...
		stmt.execute("INSERT INTO " + tableName + "(" + String.join(", ", fields) + ") VALUES ("
			+ String.join(", ", values) + ")");
		return true;
	    }
    ...
    protected Boolean delete(String idField, int id, String tableName) {
	try (Connection conn = ConnectionManager.getConnection()) {
	    try (Statement stmt = conn.createStatement()) {
    ...
    protected <T> List findBySeveralFields(String[] fields, String[] values, Map<String, String> mapColName, Class<T> clazz) {
	try (Connection conn = ConnectionManager.getConnection()) {
	    try (Statement stmt = conn.createStatement()) {
    ...
    protected <T> Boolean update(T obj, Map<String, String> colNamesMap, String tableName) {
	try (Connection conn = ConnectionManager.getConnection()) {
	    try (Statement stmt = conn.createStatement()) {
