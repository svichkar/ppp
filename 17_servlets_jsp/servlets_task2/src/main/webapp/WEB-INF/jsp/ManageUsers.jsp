			...
			<table class="submit">
			 <caption>Mange users</caption>
				...
				<c:forEach var="user" items="${users}">
					<tr>
						<form name="updateForm" id="updateForm" action="manageusers" method="post">					
							<td class="submit"><input type="text" name="userid" value="${user.userId}" readonly /></td>
							<td class="submit">
							...
							<c:choose>
								<c:when test="${user.roleId==1}">
									<td class="submit">
										<input type="text" name="selectrole" value="admin" readonly />
									</td>
								</c:when>
								<c:otherwise>
							...
							<c:choose>
								<c:when test="${user.roleId==1}">
									<td class="submit">
										<input type=submit value="edit user" name="button" onclick="changeValue(this)">
									</td>
								</c:when>
								<c:otherwise>
									<td class="submit">
									<input type=button value="edit" name="button" id="edit" onclick="changeValue(this)">
									<input type=button value="delete" name="button" id="delete" onclick="changeValue(this)">
									</td>
								</c:otherwise>
							</c:choose>
			...
			<table>
				...
				<form id="create" action="manageusers" method="post">
					<tr>
						...
						<td class="submit">
							<input type=submit value="create user" name="button">
