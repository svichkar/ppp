...
public class StatusDAOImpl implements StatusDAO<Status> {
	...
	public boolean create(Status t) {
		...
		try {
			...
			ps.execute();
			ps = conn.prepareStatement("INSERT INTO status (status_name) VALUES (?)");
	...
	public boolean update(Status t) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("UPDATE status SET status_name=?  WHERE status_id=?");
			ps.setString(1, t.getStatus_name());
			ps.setInt(2, t.getId());
			executionResult = ps.executeUpdate();
		} catch (SQLException | ClassNotFoundException ex) {
	...
	public Status findByPK(int id) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("SELECT * FROM status WHERE status_id=?");
			ps.setInt(1, id);
