...
public class CreateTables {
	...
	public static void main(String[] args) throws ClassNotFoundException {
		...
		try {
			...
			stmt.execute("CREATE TABLE student_group (group_id BIGINT PRIMARY KEY, group_name VARCHAR(256) NOT NULL);");
			...
			stmt.execute("CREATE TABLE role (role_id TINYINT PRIMARY KEY, role_name VARCHAR(256) NOT NULL);");
			LOG.info("Table role is created");
			stmt.execute("CREATE TABLE user (user_id BIGINT PRIMARY KEY, login VARCHAR(256), password VARCHAR(256),"
					+ "role_id TINYINT, FOREIGN KEY (role_id) REFERENCES role (role_id));");
			LOG.info("Table user is created");
