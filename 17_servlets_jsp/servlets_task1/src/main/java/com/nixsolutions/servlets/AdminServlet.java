...
public class AdminServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (request.getSession(false) == null || request.getSession().getAttribute("usrRole") == null
				|| !request.getSession().getAttribute("usrRole").equals("admin")) {
			...
			users = factory.getUserDao().getAllUsers();
			//page structure
			usersTable.append("<head><title>Admin page</title></head>" + "<body>" + "<h1>Welcome to the Admin page</h1>"
					+ "<p> Hi, " + request.getSession().getAttribute("usrName") + "!</p>"
					+ "<table>" + "<tr>" + "<td>user_id</td>" + "<td>user_name</td>"
					+ "<td>user_password</td>" + "<td>user_role</td>" + "<td>action</td>" + "</tr>");
			for (User user : users) {
				...
				usersTable.append(
						"<td><input type=\"text\" name=\"password\" value=\"" + user.getUserPassword() + "\"/></td>");
				if (userRole.getName().equals("admin")) {
					...
					for (Role role : allRoles) {
						if (userRole.getName().equals(role.getName())) {
							usersTable.append("<option selected value=\"" + role.getName() + "\">" + role.getName()
									+ "</option>");
						} else {
							usersTable
									.append("<option value=\"" + role.getName() + "\">" + role.getName() + "</option>");
						}
					}
			...
			usersTable.append("<form id=\"create\" action=\"createupdate\" method=\"post\">" + "<tr>" + "<td></td>"
