...
public class Arrays {
    ...
    /**
     * Performs sorting of integer array taken as an argument, in ascending
     * order, using non-optimized bubble sorting method.
     */
    public static void sort(int[] originalArray) {
        int[] a = copyArray(originalArray);
    ...
    private void print(double x, double y) {
        System.out.println(
                "Average execution times for each iteration (in seconds):\n");
        System.out.printf("com.nixsolutions.Arrays.sort: %.9f\n", x);
        System.out.printf("java.util.Arrays.sort: %.9f\n", y);
    }

    /**
     * Creates an array, measures the time of sorting it using each of the
     * methods, repeats the whole procedure multiple times, then prints average
     * time for each method.
     */
    public static void main(String[] args) {
        Arrays arrays = new Arrays();
        long[][] results = new long[2][LOOPS];
        System.out.println("Sorting arrays " + LOOPS
                + " times by each of the methods...\n");
        for (int i = 0; i < LOOPS; i++) {
            ...
            start = System.nanoTime();
            java.util.Arrays.sort(copyArray(array));
            stop = System.nanoTime();
            results[1][i] = stop - start;
        }
        arrays.print(arrays.avg(results[0]), arrays.avg(results[1]));
    }
