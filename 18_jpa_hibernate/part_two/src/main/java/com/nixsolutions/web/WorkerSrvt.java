	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (workerId > 0) {
			...
			if (action.equalsIgnoreCase("Edit")) {
				...
				worker.setSpec(workerspecImpl.findByPK(specId));
				worker.setStatus(statusImpl.findByPK(statusId));
			...
			if (action.equalsIgnoreCase("Add")) {
				...
				Worker worker = new Worker(f_name, l_name, workerspecImpl.findByPK(specId),
						statusImpl.findByPK(statusId));
