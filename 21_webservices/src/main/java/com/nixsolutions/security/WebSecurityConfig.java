...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configAuthentication(AuthenticationManagerBuilder auth) throws Exception {
		...
		.usersByUsernameQuery("select username, password, 'true' "
				+ "from sqllab.user inner join sqllab.role on user.role_id=role.role_id "
				+ "where username=?")
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			.authorizeRequests()
			.antMatchers("/navigation","/car*","/customer*", "/order*", "/part*", "/user*", "/worker*").access("hasRole('ROLE_ADMIN')").anyRequest().permitAll()
			.antMatchers("/userpage.jsp", "/logout", "/login").access("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')").anyRequest().permitAll()
			.and()
			.formLogin()
			.loginPage("/")
