...
public class DatesExp {
	...
	public static void getMondays(Locale def, Date df) {
		...
		for (int i = 1; i < cal.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
			if (cal.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG, def)
					.equals("Monday")) {
				System.out.println(
						DateFormat.getDateInstance(DateFormat.FULL, def)
								.format(cal.getTime()));
			}
	...
	public static void fridayThirteen(Locale def, Date df) {
		...
		cal.setTime(df);
		if (cal.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG, def)
				.equals("Friday") && cal.get(Calendar.DAY_OF_MONTH) == 13) {
	...
	public static void howLong(Locale def, Date df) {
		...
		long days = (timeDiff
				- ((years * 31536000000L) + (months * 2592000000L)))
				/ 86399000L;
		System.out.println(
