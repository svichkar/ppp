...
public class TestOrderStatusDAO extends DBTestCase{
	...
	public static Logger LOG = LogManager.getLogger(TestOrderStatusDAO.class.getName());
	...
	public void testOrderStatusDAOAddsEntity() throws DataSetException, Exception {
		...
		OrderStatus o = new OrderStatus(0, "test");
		OrderStatus tOrderStatus = orderStatusDAO.createFrom(o);
		IDataSet dbSet = getConnection().createDataSet(new String[] {"ORDER_STATUS"});
		ITable dbTable = dbSet.getTable("ORDER_STATUS");
		Assert.assertEquals(tOrderStatus.getOrderStatusId(), dbTable.getValue(dbTable.getRowCount() - 1, "order_status_id"));
