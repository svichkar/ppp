...
<form id="find" action=findreader method="post">
				...
				<input class="submit" type="text" name="search input"
					placeholder="Reader name"
					data-tooltip="here my advice will be shown" /> 
				...
				<tr>
					...
					<td class="present"><form id="${reader.clientId}"
								...
								<input type="hidden" name="current client"
										value="${reader.clientId}" /><input type=submit
										value="manage" name="button">
							<c:if test="${not empty toBeloaned}">
						<c:forEach var="book" items="${toBeloaned}">
							<input type="hidden" name="loaned" value="${book.bookId}"
