...
<t:general_template title="loan book">
	<jsp:attribute name="content_area">
		...
		<form id="find" action=readersearch method="post">
			<input type="text" name="search input" placeholder="Reader name" /> 
			...
			<table class="present">
				...
				<c:forEach var="reader" items="${readers}">				
					<tr>
						...
						<td class="present">
							<form id="${reader.clientId}" action=loansmanage method="post">
								<input type="hidden" name="current client" value="${reader.clientId}" />
								<input type=submit value="manage" name="button">
								<c:if test="${not empty toBeloaned}">
									<c:forEach var="book" items="${toBeloaned}">
										<input type="hidden" name="loaned" value="${book.bookId}" form="${reader.clientId}" />
									</c:forEach>		
								</c:if>	
							</form>
						</td>
