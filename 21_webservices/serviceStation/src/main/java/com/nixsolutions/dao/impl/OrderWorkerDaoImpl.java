...
public class OrderWorkerDaoImpl implements OrderWorkerDao {
	 ...
	 */
	@Override
	public OrderWorker getOrderWorker(Integer orderId, Integer workerId){
		try {
			return (OrderWorker) sessionFactory.getCurrentSession().createCriteria(OrderWorker.class, "oWorker")
					...
					.add(Restrictions.eq("order.order_id", orderId))
					.add(Restrictions.eq("worker.worker_id", workerId)).uniqueResult();
		} catch (Exception ex) {
			logger.error(ex);
		}
		return null;
	...
	public OrderInWork getActiveOrderByWorkerID(Integer workerId) {
		...
		try {
			orderWorker = (OrderWorker) sessionFactory.getCurrentSession().createCriteria(OrderWorker.class)
					.add(Restrictions.eq("worker.worker_id", workerId)).add(Restrictions.eq("isCompleted", false))
					.uniqueResult();
		} catch (Exception ex) {
			logger.error(ex);
		}
	...
	public List<OrderWorker> getWorkersByOrderID(Integer orderId) {
		...
		try {
			order_workers = sessionFactory.getCurrentSession().createCriteria(OrderWorker.class, "orWork")
					.createAlias("orWork.order", "order").add(Restrictions.eq("order.orderId", orderId)).list();
		} catch (Exception ex) {
			logger.error(ex);
		}
		return order_workers;
