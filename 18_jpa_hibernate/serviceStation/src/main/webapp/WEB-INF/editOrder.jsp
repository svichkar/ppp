			...
			<tr>
						...
						<table class="nav-menu">
							<c:forEach var="oPart" items="${parts}">
								<tr>
									...
									<td class="nav-menu">
									<input hidden="hidden" name="order_id" value="${oPart.order.order_id}" />
									<input hidden="hidden" name="part_id" value="${oPart.part.part_id}" />
				...
				<td class="nav-menu"><form id="order" action="updateField"
						...
						<table class="data">
							...
							<tr>
								<th>Order Status</th>
								<td><b>Actual: <c:out value="${order.order_status.order_status_id}"></c:out></b><br /><select
					...
					<form id="workerInOrder" action="updateWorkerOrder" method="post">
						...
						<table class="nav-menu">
							<c:forEach var="oWorker" items="${workers}">
								<tr>
								<c:choose>
								<c:when test="${oWorker.worker.worker_status.worker_status_name == 'busy'}">
									<td class="nav-menu"><p>
											<c:out value="${oWorker.worker.last_name }"></c:out><c:out value=" ${oWorker.worker.first_name }"></c:out>
										</p></td>
									<td class="nav-menu">
									<p>
											<c:out value="${oWorker.worker.specialization.specialization_name }"></c:out>
										</p></td>
									<td class="nav-menu">
									<p>
											<c:out value="${worker.worker.worker_status.worker_status_name}"></c:out>
										</p></td>
									...
									<td class="nav-menu">
									<input hidden="hidden" name="worker_id" value="${oWorker.worker.worker_id}" />
									<input hidden="hidden" name="order_id" value="${oWorker.order.order_id}" />
									<input type="submit" value="update">
					...
					<form id="orderWorker" action="updateWorkerOrder" method="post">
						...
						<select	name="worker_id" >
							<c:forEach var="worker" items="${allWorkers}">
								...
								<c:out value="${worker.last_name}"></c:out> <c:out value="${worker.first_name}"></c:out>	SPEC:<c:out
