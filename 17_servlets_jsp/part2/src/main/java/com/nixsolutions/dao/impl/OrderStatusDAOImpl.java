...
public class OrderStatusDAOImpl implements OrderStatusDAO<OrderStatus> {
	...
	public boolean create(OrderStatus t) {
		...
		try {
			...
			ps.execute();
			ps = conn.prepareStatement("INSERT INTO order_status (order_status_name) VALUES (?)");
	...
	public boolean update(OrderStatus t) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("UPDATE order_status SET order_status_name=? WHERE order_status_id=?");
			ps.setString(1, t.getOrder_status_name());
			ps.setInt(2, t.getId());
			executionResult = ps.executeUpdate();
		} catch (SQLException | ClassNotFoundException ex) {
	...
	public boolean delete(OrderStatus t) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("DELETE FROM order_status WHERE order_status_id=?");
			ps.setInt(1, t.getId());
			executionResult = ps.executeUpdate();
		} catch (SQLException | ClassNotFoundException ex) {
			LOG.error(ex, ex);
	...
	public OrderStatus findByPK(int id) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("SELECT * FROM order_status WHERE order_status_id=?");
			ps.setInt(1, id);
			...
			if (resultExecution) {
				...
				while (rs.next()) {
					...
					orderstatus.setOrder_status_name(rs.getString("order_status_name"));
				}
			} else {
				orderstatus = null;
			}
