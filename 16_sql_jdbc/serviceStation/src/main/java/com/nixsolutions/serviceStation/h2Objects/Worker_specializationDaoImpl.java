...
public class Worker_specializationDaoImpl implements Worker_specializationDao {
	...
	public void deleteTableWithAllData() {
		try {
			...
			PreparedStatement stmt = dbConnector.getConnection()
					.prepareStatement("DROP TABLE worker_specialization ;");
			int set = stmt.executeUpdate();
			if (set == 1)
	...
	public List<Worker_specialization> getAllSpecialization() {
		...
		try {
			...
			PreparedStatement stmt = dbConnector.getConnection().prepareStatement("SELECT * FROM worker_specialization");
			ResultSet set = stmt.executeQuery();
			dbConnector.closeConnection();
			logger.trace("Generate list of the worker_specialization objects");
			while (set.next()) {
				specializations.add(new Worker_specialization(set.getInt("specialization_id"), set.getString("specialization_name")));
			}
			stmt.close();
		} catch (SQLException e) {
			logger.error(e.getMessage(), e);
