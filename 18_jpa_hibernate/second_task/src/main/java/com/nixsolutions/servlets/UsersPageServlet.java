		...
		if (request.getParameter("add") != null) {
			...
			if (loginList.contains(request.getParameter("newLogin"))) {
				...
				if (emailList.contains(request.getParameter("newEmail"))) {
					...
					newUser.setRole(roleDao.findRoleById(Long.valueOf(request.getParameter("newRole"))));							
					userDao.createUser(newUser);
					response.sendRedirect("users?message=Added user with id " + newUser.getUserId());
				}
			}
		}
	}
