...
public class WorkerSpecificationDAOImpl implements WorkerSpecificationDAO<WorkerSpecification> {
	...
	public boolean create(WorkerSpecification t) {
		...
		try {
			...
			ps.execute();
			ps = conn.prepareStatement("INSERT INTO worker_specification (spec_name) VALUES (?)");
	...
	public boolean update(WorkerSpecification t) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("UPDATE worker_specification SET spec_name=? WHERE spec_id=?");
			ps.setString(1, t.getSpec_name());
			ps.setInt(2, t.getId());
			executionResult = ps.executeUpdate();
		} catch (SQLException | ClassNotFoundException ex) {
	...
	public boolean delete(WorkerSpecification t) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("DELETE FROM worker_specification WHERE spec_id=?");
	...
	public WorkerSpecification findByPK(int id) {
		...
		try {
			...
			ps = conn.prepareStatement("SET SCHEMA SQLLAB");
			ps.execute();
			ps = conn.prepareStatement("SELECT * FROM worker_specification WHERE spec_id=?");
			ps.setInt(1, id);
