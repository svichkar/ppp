...
<t:template title="Update User">
	...
	<jsp:attribute name="content_area">
       <h2>Update User</h2>
       <form action="updateUser" method="post" onsubmit="return validate(this, optionsForUpdateUser)">
       ...
       <td><input type="text" name="updatedLogin"
						value="<c:out value="${updateUser.login}"/>" required>
						<span class="hint">Update login in text format</span></td>
				</tr>
		<tr>
		<td>Email : </td>
		<td><input type="text" name="updatedEmail"
						value="<c:out value="${updateUser.email}"/>" required>
						<span class="hint">Update email in format xxx@xxx.xxx</span></td>
				</tr>
				<tr>
		<td>Password : </td>
		<td><input type="text" name="updatedPassword"
						value="<c:out value="${updateUser.password}"/>" required>
						<span class="hint">Update password</span></td>
				</tr>
		<tr>
		<td>Role : </td>
       ...
       <option value="<c:out value="${role.roleId}"/>"><c:out
			...
			<span class="hint">Select role from the list</span></td>
				</tr>
		</table>
