...
public class CarRestWebService {
	...
	public Response addCar(@RequestBody CarCustomer carCustomer) {
		...
		if (carCustomer != null) {
			customer = customerServiceImpl.getCustomerById(carCustomer.getCustomerId());
		} else {
			customer = new Customer(carCustomer.getFname(), carCustomer.getLname(), "", null);
			customerServiceImpl.addCustomer(customer);
			customer = customerServiceImpl.getCustomerByFullName(carCustomer.getFname(), carCustomer.getLname());
		}
		...
		return Response.status(httpStatus).entity(resultId).build();
	}
	...
	public Response updateCar(@RequestBody CarCustomer carCustomer) {
		...
		int httpStatus = 500;
		if (carCustomer != null) {
			// long parsedCarId = carCustomer.getId() > 0 ? carCustomer.getId()
			// : Long.valueOf(carId);
